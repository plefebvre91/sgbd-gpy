...
\indent En outre, il faut gérer correctement la suppression de données de la base. Par exemple, lorsqu'un jeu est supprimé de la base, il faut supprimer les associations jeu-catégorie et jeu-plateforme correspondantes. Cela est possible avec la syntaxe suivante en MySQL: \\
...
\indent Pour conclure, réaliser une base de données impose de faire certains choix. Une des choses fondamentales qui n'a pas été prise en compte dans ce travail est les cas d'utilisation. En effet, selon les opérations qui seront le plus effectuées sur la base de données, la modélisation peut changer pour optimiser ces opérations. C'est cependant délicat d'imaginer le comportement des utilisateurs de la base sans étude préalable.
