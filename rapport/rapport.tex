...
\indent Plusieurs problèmes se sont alors posés. Une première idée a été de mettre en clé primaire le couple (idJeu, idPlateforme) pour la table commentaire. Cependant, ce choix n'a finalement pas été retenu pour plus de simplicité, et un idCommentaire a été ajouté. Par ailleurs, le choix initial de mettre un idJoueur a posé problème vis-à-vis d'une des contraintes explicitée auparavant. Avec un pseudo unique, il y avait rédondance de l'information. Le pseudo du joueur a alors constitué sa clé primaire. Au final voici le modèle relationnel retenu. \\
...
\indent A noter que le modèle ci-dessus n'est pas en troisième forme normale. En effet, il est possible de connaître le nom et le prénom d'un joueur à partir de son adresse mail, qui n'est pas une clé primaire. Pour éviter ce problème, il aurait fallut créer une table informations en association avec la table joueur et contenant son nom et son prénom. Cependant, après discussion avec notre encadrant de projet, cette solution n'a pas été retenue pour plus de clarté quand à l'entité joueur.
